...
# check if the script already installed
scriptname='sshvpn';
...
if [ "$scriptchecker" != "" ]; then
		clear
		echo -e " ";
		echo -e "Error! You've already run this script before";
		echo -e "This script that can be used only once!";
		echo -e "---";
		echo -e "If the installation failed, Please reinstall your VPS OS first!";
		echo -e "You can reinstall your VPS OS via VPS Control Panel";
		echo -e "How to Access VPS Control Panel: bit.ly/caraaksesvpspanel";
		echo -e " ";
        exit 0;
	else
		echo "";
...
sed -i 's|export KEY_NAME=changeme|export KEY_NAME=0123456|' /etc/openvpn/easy-rsa/2.0/vars
...
# install badvpn
wget -O /usr/bin/badvpn-udpgw "https://github.com/daybreakersx/premscript/raw/master/badvpn-udpgw"
if [ "$OS" == "x86_64" ]; then
  wget -O /usr/bin/badvpn-udpgw "https://github.com/daybreakersx/premscript/blob/master/badvpn-udpgw64"
fi
